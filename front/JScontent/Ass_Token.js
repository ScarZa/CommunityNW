function AssToken(content, id = null) {
    var RL = new ReportLayout(content);
    RL.GetRL();
    var title = " อ่าน JWT Token Key ";
    $("nav#nav_bc").hide();
    ...
    $("input#relative").keyup(function () {
        var tokenKeyHead = parseJwtHead($("input#relative").val())
        console.log(tokenKeyHead);
    var tokenKey = parseJwt($("input#relative").val())
      console.log(tokenKey);
      var secret = "ScarZ.Ltd";
        if (tokenKeyHead.alg == 'HS256') {
            ...
            var new_signature = base64url(CryptoJS.HmacSHA256(encodedHeader + "." + encodedData, secret));
            var ref_signature = $("input#relative").val().split('.')[2];
            console.log($("input#relative").val().split('.')[2])
            ...
            if (ref_signature == new_signature) {
                var header = {
                  ...
                  var encodedHeaderen = base64url(stringifiedHeaderen);
                  
                  var dataen = tokenKey;
                  
                  var stringifiedDataen = CryptoJS.enc.Utf8.parse(JSON.stringify(dataen));
                  var encodedDataen = base64url(stringifiedDataen);
                  
                  var token = encodedHeaderen + "." + encodedDataen;
                  
                  var secreten = "ScarZ.Ltd";
            
            var signature = CryptoJS.HmacSHA256(token, secreten);
            ...
            console.log('ลายเซ็นต์ผิดครับ');
        }
     }else {
        console.log('Algorithm ไม่ใช่ HS256');
    }
        
      
  });
            $.getJSON('http://10.0.0.11/API-Hosxp/API/detail_CCpatientAPI.php',{data : $.cookie("vn"),data2 : $.cookie("an")},function (data) {
            $("#P-data").append($("<div class='row col-lg-6'><span id='DP'>HN : "+data[0].hn+"<br>เลขบัตรประชาชน : "+data[0].cid+"<br>ชื่อ-สกุล :"+data[0].fullname
                                    ...
                                    +"<div class='col-lg-6 block'> <img src='http://10.0.0.11/API-Hosxp/API/show_image.php?hn="+$.cookie("hn")+"' width='150' /></div>")
        ...
        $("#frmER").on('submit', (function (e) {
            ...
            var settings = {
                type: "POST",
                url: "http://10.0.0.11/API-Hosxp/API/prcERAPI.php",
                async: true,
                crossDomain: true,
                data: dataForm,
                contentType: false,
